...
class ClassificadorXML:
    ...
    def _carregar_empresas_monitoradas(self):
        ...
        print("[DEBUG] Lista de CNPJs monitorados:", self.empresas_monitoradas)
    ...
    def extrair_metadados_nfe(self, root: ET.Element, ns: Dict[str, str]) -> DocumentoMetadata:
        ...
        try:
            ...
            # DEBUG: Logar o que foi extraído
            print(f"[DEBUG] (extrair_metadados_nfe) CNPJ emitente extraído: '{metadata.cnpj_emissor}' | CNPJ destinatário extraído: '{metadata.cnpj_destinatario}' | ns: {ns}")
    ...
    def _get_text_element(self, parent: ET.Element, tag: str, ns: Dict[str, str]) -> Optional[str]:
        """Busca texto de um filho direto, tentando com e sem namespace"""
        element = parent.find(f'ns:{tag}', ns) or parent.find(tag)
    ...
    def classificar_documento(self, metadata: DocumentoMetadata) -> str:
        ...
        # Regras para NF-e
        print(f"[DEBUG] CNPJ emissor extraído: '{metadata.cnpj_emissor}'")
        print(f"[DEBUG] CNPJ destinatário extraído: '{metadata.cnpj_destinatario}'")
        print(f"[DEBUG] CNPJs monitorados: {self.empresas_monitoradas}")
        ...
        print(f"[DEBUG] Emissor monitorado? {emissor_monitorado} | Destinatário monitorado? {destinatario_monitorado}")
    ...
    def processar_xml(self, xml_path: Path, arquivo_origem: str = None) -> DocumentoMetadata:
        ...
        try:
            ...
            # DEBUG: Mostrar CNPJ extraído e lista de monitorados antes da classificação
            print(f"[DEBUG] XML: {xml_path} | CNPJ emissor extraído: '{getattr(metadata, 'cnpj_emissor', None)}' | CNPJ destinatário extraído: '{getattr(metadata, 'cnpj_destinatario', None)}'")
            print(f"[DEBUG] CNPJs monitorados: {getattr(self, 'empresas_monitoradas', None)}")
        ...
        try:
            with SessionLocal() as db:
                ...
                if not exists:
                    ...
                    chave_nfe_log = metadata.chave_acesso or ""
                    motivo_log = metadata.motivo_classificacao or ""
