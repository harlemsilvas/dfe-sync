...
async def list_xmls_nao_classificados():
    ...
    with SessionLocal() as db:
        ...
        import logging
        logger = logging.getLogger("xmls_nao_classificados")
        arquivos_encontrados = list(xml_path.glob("*.xml")) if xml_path.exists() else []
        logger.info(f"[DEBUG] Arquivos encontrados: {[str(f) for f in arquivos_encontrados]}")
        if xml_path.exists():
            for xml_file in arquivos_encontrados[:50]:  # Limite de 50 para performance
                try:
                    stat = xml_file.stat()
                    ...
                    logger.error(f"[DEBUG] Erro ao processar {xml_file}: {e}")
                    continue
        logger.info(f"[DEBUG] Total xmls_nao_classificados: {len(xmls_nao_classificados)}")
        return {
